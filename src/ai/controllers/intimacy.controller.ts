...
import { GetIntimacyResponseDto } from '../dto/intimacy.dto';
...
export class IntimacyController {
  ...
  async getIntimacy(
    ...
    try {
      ...
      // 친밀도 레벨 계산
      const intimacyLevel =
        await this.intimacyCalculatorService.calculateIntimacyLevel({
          totalTodos: userData.recentTodos.length,
          weeklyRetrospects: userData.weeklyRetrospects.length,
        });

      const intimacyInfo =
        this.intimacyCalculatorService.getIntimacyInfo(intimacyLevel);
      ...
      this.logger.error(`친밀도 조회 실패:`, error.message);
