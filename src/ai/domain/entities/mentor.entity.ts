...
export class Mentor {
  ...
  generateAdviceContext(
    todos: string[],
    retrospects: string[],
    intimacyLevel: IntimacyLevel,
  ): AdviceContext {
    const intimacyModifier = this.getIntimacyModifier(intimacyLevel);
  ...
  private getIntimacyModifier(intimacyLevel: IntimacyLevel): string {
    return match(intimacyLevel)
      .with(
        IntimacyLevel.HIGH,
        () => ' 더욱 구체적이고 도전적인 조언으로 답변해줘.',
      )
      .with(IntimacyLevel.MEDIUM, () => '')
      .with(IntimacyLevel.LOW, () => ' 기본적이고 격려 위주로 답변해줘.')
      .exhaustive();
  }
